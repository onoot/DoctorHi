...
async function loadTransactionPayments(transactionId) {
  try {
    // Запрашиваем ВСЮ информацию о транзакции
    const response = await apiRequest(`/v1/admin/transactions/${transactionId}`);
    ...
    if (response && response.transaction && Array.isArray(response.transaction.payments)) {
      ...
      payments.forEach(payment => {
        ...
        if (payment.receipt && payment.receipt.path) {
          ...
          if (payment.receipt.path.toLowerCase().endsWith('.pdf')) {
            receiptPreview = `
            ...
            `;
          } else if (payment.receipt.path.toLowerCase().match(/\.(jpg|jpeg|png|gif)$/)) {
            receiptPreview = `
              <div class="receipt-preview">
